    ...
    try:
        ...
        <html>
            <body style="font-family: Arial, sans-serif;">
                <h2 style="color: #FF4B4B;">🚨 Disaster Relief Alert</h2>
...
with st.sidebar.expander("🌐 API Settings", expanded=False):
    ...
    if use_live_weather and not api_key_input:
        st.warning("⚠️ Please enter API key to use live weather")
...
with st.sidebar.expander("🚨 Alert Configuration", expanded=False):
    ...
    if enable_email:
        ...
        if st.button("✉️ Test Email"):
            if gmail_user and gmail_password and alert_email:
                ...
                <ul>
                    <li>🔴 IMMEDIATE priority disasters</li>
                    <li>🟠 Resource shortages below 70%</li>
                    <li>⚠️ Medical facility gaps</li>
                    <li>⛈️ Extreme weather conditions</li>
                </ul>
                """
    ...
    st.markdown("---")
    st.markdown("### 📱 SMS Settings (Twilio)")
    
    enable_sms = st.checkbox("Enable SMS Alerts", value=True)
    
    if enable_sms:
        st.info("🔐 Using secure credentials from Streamlit Secrets")
        ...
        try:
            ...
            st.error("⚠️ Twilio credentials not configured. Contact admin.")
            st.stop()
        ...
        st.caption("💡 Include country code (India: +91, US: +1)")
        
        # Test SMS Section
        st.markdown("#### 🧪 Test SMS Alert")
        ...
        with col2:
            ...
            if st.button("📲 Send Test SMS", use_container_width=True):
                if test_phone and len(test_phone) > 5:
                    ...
                    test_message = "🚨 Test Alert - Disaster Relief System is active! You will receive emergency notifications. Status: OK"
                    ...
                    with st.spinner("📤 Sending SMS..."):
                        ...
                        if success:
                            ...
                            st.error(f"❌ {msg}")
                            st.info("💡 Check: 1) Phone format (+country code), 2) Twilio account balance")
                else:
        ...
        with status_col1:
            email_status = "🟢 Active" if enable_email and gmail_user else "⚪ Inactive"
            st.markdown(f"**Email:** {email_status}")
        
        with status_col2:
            sms_status = "🟢 Active" if enable_sms and alert_phone else "⚪ Inactive"
            st.markdown(f"**SMS:** {sms_status}")
        
        if enable_sms and alert_phone:
            st.success(f"✅ Alerts will be sent to: {alert_phone}")
    else:
        st.session_state.alert_settings = {
            'email_enabled': False,
            'sms_enabled': False,
...
if use_live_weather and st.session_state.api_key:
    ...
    with st.spinner("🌐 Fetching live weather data..."):
        ...
        if live_weather is not None and not live_weather.empty:
            st.sidebar.success(f"✅ Live data updated for {len(live_weather)} cities")
            disaster_data = merge_live_and_static_data(disaster_data, live_weather)
        else:
            st.sidebar.warning("⚠️ Using cached data")
...
# Title
st.markdown('<div class="main-header">🚨 AI-Powered Disaster Relief Management System</div>', unsafe_allow_html=True)
...
if page == "🏠 Dashboard Overview":
    ...
    if use_live_weather and st.session_state.api_key:
        st.success("🔴 LIVE DATA MODE - Real-time weather monitoring active")
    else:
        st.info("📊 STATIC DATA MODE - Using historical data")
    ...
    with col4:
        ...
        st.metric(
            label="📦 Avg Resource Fulfillment",
    ...
    # Live Data Indicator
    if use_live_weather and st.session_state.api_key:
        st.info(f"🔴 Live data - Last updated: {st.session_state.last_update.strftime('%H:%M:%S')}")
...
# ============================================================================
    ...
    st.header("🚨 Active Alerts & Notifications")
    ...
    with col2:
        st.metric("🟠 High Priority Alerts", high_count)
    with col3:
        sms_enabled = st.session_state.alert_settings.get('sms_enabled', False)
        st.metric("📧 Alert System", "ACTIVE" if sms_enabled else "INACTIVE")
    ...
    if st.session_state.alert_settings.get('sms_enabled', False):
        ...
        with col1:
            st.success(f"✅ SMS Alerts configured for: {alert_phone}")
        
        with col2:
            if st.button("📲 Send All Critical Alerts", use_container_width=True, type="primary"):
                sent_count = 0
                failed_count = 0
                ...
                with st.spinner("📤 Sending alerts..."):
                    for alert in alerts:
                        if alert['severity'] == 'CRITICAL':
                            ...
                            sms_message = f"🚨 CRITICAL: {alert['city']} - {alert['message'][:80]} Alert:{alert['alert_id']}"
                ...
                if sent_count > 0:
                    st.success(f"✅ Sent {sent_count} critical alert(s) to {alert_phone}!")
                    st.balloons()
                
                if failed_count > 0:
                    st.warning(f"⚠️ {failed_count} alert(s) failed to send")
    else:
        st.warning("⚠️ SMS alerts not configured. Enable in sidebar settings.")
        st.info("👈 Configure alerts in the sidebar under '🚨 Alert Configuration'")
    ...
    with col2:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("📲 Send Selected Alert", use_container_width=True):
            if custom_phone and alert_to_send and alerts:
                ...
                if selected_idx < len(alerts):
                    ...
                    sms_message = f"🚨 {alert['severity']}: {alert['city']} - {alert['message'][:70]} {alert['alert_id']}"
                    ...
                    with st.spinner("📤 Sending..."):
                        ...
                        if success:
                            st.success(f"✅ Alert sent to {custom_phone}!")
                        else:
                            st.error(f"❌ {msg}")
    ...
    # Display all alerts
    st.subheader(f"📋 Active Alerts ({len(alerts)} total)")
    
    if len(alerts) == 0:
        st.info("✅ No critical alerts at this time. All systems normal.")
    ...
    for alert in alerts:
        ...
        with st.container():
            if severity_type == 'error':
                st.error(f"**{alert['alert_id']} | 🔴 {alert['severity']} | {alert['city']}**")
            elif severity_type == 'warning':
                st.warning(f"**{alert['alert_id']} | 🟠 {alert['severity']} | {alert['city']}**")
            else:
                st.info(f"**{alert['alert_id']} | 🟡 {alert['severity']} | {alert['city']}**")
            ...
            with col3:
                ...
                st.write(f"⚡ Score: {alert['priority_score']:.1f}")
            
            # Quick send button for this alert
            if st.session_state.alert_settings.get('sms_enabled', False):
                if st.button(f"📲 Send {alert['alert_id']}", key=f"send_{alert['alert_id']}"):
                    # SHORT message for trial
                    sms_text = f"🚨 {alert['severity']}: {alert['city']} - {alert['message'][:90]}"
                    success, msg = send_sms_alert(
...
# Add placeholder for other pages
elif page == "📍 Disaster Data":
    st.header("📍 Disaster Data Overview")
    ...
    st.header("🗺️ Interactive Disaster Map")
...
st.markdown(f"""
    <div style='text-align: center; color: gray; padding: 20px;'>
        <p>🚨 AI-Powered Disaster Relief Management System</p>
        <p>Status: {'🔴 LIVE' if use_live_weather and st.session_state.api_key else '📊 STATIC'} | 
        Alerts: {'✅ ACTIVE' if st.session_state.alert_settings.get('sms_enabled', False) or st.session_state.alert_settings.get('email_enabled', False) else '❌ DISABLED'} | 
